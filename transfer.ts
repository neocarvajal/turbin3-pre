...
// Turbin3 wallet to send to
const turbin3Wallet = address("8H8nCS6JUhKNJRHbC2fmr6ofHRLsYCapqVmb5CJJ6VE6");

async function transactaionEx(): Promise<void> {
...
async function cleanAccount(): Promise<void> {
    const keypair = await createKeyPairSignerFromBytes(new Uint8Array(wallet));
    ...
    const dummyMessageBase64 = Buffer.from(
        compiledDummy.messageBytes)
